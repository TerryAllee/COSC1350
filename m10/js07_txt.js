...
/*    JavaScript 7th Edition
      ...
      Filename:       js07_txt.js

*/


 // Add the event handler for the file input
document.getElementById("getFile").onchange = function() {
      ...
      try {
          ...
          // Once the file is finished loading, display it on the page
          fr.onload = function() {
  ...
  function wordCloud(sourceText) {
      // Convert the source text to lowercase and trim whitespace
      ...
      // Create a 2D array in which each item is an array containing a word and its duplicate count
      let unique = [[words[0], 1]]; // Start with the first word and count 1
  
      // Keep an index of the unique words
      let uniqueIndex = 0;
  
      // Loop through the sorted words array to count duplicates
      for (let i = 1; i < words.length; i++) {
          if (words[i] === words[i - 1]) {
              // Increase the duplicate count by 1
              unique[uniqueIndex][1]++;
          } else {
              // Add a new word to the unique array with count 1
              uniqueIndex++;
      ...
      // Define the byDuplicate function for sorting
      ...
      // Find the maximum duplicate count to scale font sizes
      ...
      for (let i = 0; i < unique.length; i++) {
          // Create a span element containing the text of every word
          let word = document.createElement("span");
          word.textContent = unique[i][0];
  
          // Set the font size as a percentage of the largest duplicate count
          word.style.fontSize = (unique[i][1] / maxCount) * 3 + "em"; // Adjust scaling factor as needed
  
          // Append the word to the word cloud box
          cloudBox.appendChild(word);
      }
  
      // Log the unique words and their counts to verify
      console.log(unique);
  }
